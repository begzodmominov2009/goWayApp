...
/// Tuman/shahar tanlash — mustaqil sahifa (Navigator.push orqali,
/// tanlangan viloyat ichida). Natija: Navigator.pop(context, district) —
/// faol tuman tanlanganda, aks holda null.
class DistrictPickerPage extends ConsumerStatefulWidget {
  final String title;
  final GeoRegion region;
  const DistrictPickerPage({super.key, required this.title, required this.region});

  @override
  ConsumerState<DistrictPickerPage> createState() => _DistrictPickerPageState();
}

class _DistrictPickerPageState extends ConsumerState<DistrictPickerPage> {
  bool _loading = true;
  ...
  Future<void> _load() async {
    final list = await ref.read(geoRepositoryProvider).getAllDistricts(widget.region.id);
  ...
  void _select(GeoDistrict district) {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _districts.where((d) {
      if (_filter == _ActiveFilter.activeOnly && !d.isActive) return false;
      if (_filter == _ActiveFilter.inactiveOnly && d.isActive) return false;
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: AppStrings.get('district_search_hint', locale),
